#!/bin/bash
# Mac M4 软件包自动更新脚本 - kt-tools 版

set -uo pipefail

SCRIPT_DIR=$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)
ROOT_DIR=$(cd "$SCRIPT_DIR/.." && pwd)
CACHE_DIR="$ROOT_DIR/resources/software-cache"
...
update_from_github() {
    ...
    local official_page="${6:-https://github.com/$repo/releases/latest}"
    local url

    url=$(get_github_asset_url "$repo" "$pattern") || true
    if [[ -z "$url" ]]; then
        log_line "✗ $name 未找到匹配 $pattern 的 GitHub 资源"
        FAILED_UPDATES+=("$name: 未找到资源")
        return 1
    fi
...
update_nodejs_pkg() {
    ...
    if ! update_software "$name" "$url" "$filename" "$min_size" "Node.js 官方发布通道 https://nodejs.org/dist/latest/"; then
        FAILED_UPDATES+=("$name: 下载失败")
        return 1
    fi
...
update_trae_pkg() {
    ...
    local official_page="${4:-https://api.trae.ai/icube/api/v1/native/version/trae/latest}"
    local url
...
get_latest_homebrew_pkg_url() {
    ...
    mapfile -t headers < <(github_headers)
    curl -sL "${headers[@]}" "$api" |
        grep -oE '"browser_download_url"\s*:\s*"[^"]+\.pkg"' |
        head -1 |
        cut -d'"' -f4
}
...
update_homebrew_pkg() {
    ...
    local official_page="${4:-https://github.com/Homebrew/brew/releases/latest}"
    local url
...
update_git_pkg() {
    ...
    local filename="$2"
    local min_size="${3:-50000000}"
    local official_page="${4:-https://sourceforge.net/projects/git-osx-installer/}"
    local url
    ...
    if update_software "$name" "$url" "$filename" "$min_size" "$official_page"; then
        return 0
    fi

    log_line "✗ $name 主下载失败，尝试备用镜像"
    if update_software "$name" "https://mirrors.tuna.tsinghua.edu.cn/github-release/git/git/LatestRelease/Git-OSX-Installer-2.47.0-Intel-universal-mavericks.dmg" "$filename" 20000000 "$official_page"; then
        return 0
    fi

    log_line "✗ $name 镜像下载失败"
    FAILED_UPDATES+=("$name: 镜像失败")
    return 1
}

log_line "开始检查软件更新..."

update_software "ChatGPT" "https://persistent.oaistatic.com/sidekick/public/ChatGPT.dmg" "ChatGPT_M.dmg" 40000000 "OpenAI 官方 CDN https://persistent.oaistatic.com/sidekick/public/ChatGPT.dmg"
sleep 2

update_software "Telegram" "https://osx.telegram.org/updates/Telegram.dmg" "Telegram_M.dmg" 80000000 "Telegram 官方桌面更新源 https://osx.telegram.org/updates/Telegram.dmg"
sleep 2

update_software "Google Chrome" "https://dl.google.com/chrome/mac/universal/stable/GGRO/googlechrome.dmg" "Chrome_M.dmg" 150000000 "Google 官方稳定版通道 https://dl.google.com/chrome/mac/universal/stable/GGRO/googlechrome.dmg"
sleep 2

update_software "Docker Desktop" "https://desktop.docker.com/mac/main/arm64/Docker.dmg" "Docker_M.dmg" 300000000 "Docker 官方 ARM64 构建 https://desktop.docker.com/mac/main/arm64/Docker.dmg"
sleep 2
